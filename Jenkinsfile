...
pipeline {
    ...
    stages {
        ...
        stage('Run Docker Image') {
            steps {
                sh "docker run --name $CONTAINER_NAME --restart always --env NEXT_PUBLIC_KSOS_API_URL=$NEXT_PUBLIC_KSOS_API_URL --env NEXT_PUBLIC_RIOT_DRAGON_VERSION=$NEXT_PUBLIC_RIOT_DRAGON_VERSION -p 3001:3000 -d $REGISTRY:latest"
            }
        }
    }
}